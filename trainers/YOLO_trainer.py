...
#         """
#         Відображає меню вибору версії YOLO, її розміру та режиму навчання.
#         """
...
#         print("\n   Оберіть розмір моделі (впливає на точність та швидкість):")
#         print("     1: n (nano)   - максимальна швидкість")
#         print("     2: s (small)  - хороший баланс")
#         print("     3: m (medium) - вища точність, помірна швидкість (рекомендовано для 640px)")
#         print("     4: l (large)  - висока точність")
#         print("     5: x (xlarge) - максимальна точність")
        
#         size_map = {'1': 'n', '2': 's', '3': 'm', '4': 'l', '5': 'x'}
#         size_choice = None
#         while size_choice not in size_map:
#             size_choice = input("   Ваш вибір розміру (1-5): ").strip()
#             if size_choice not in size_map:
#                  print("   ❌ Невірний вибір. Будь ласка, введіть число від 1 до 5.")
#         model_size_suffix = size_map[size_choice]
#         # Для YOLOv9 немає 'n' та 's', замінимо їх на 'c' (compact)
#         if model_choice == '2' and model_size_suffix in ['n', 's']:
#             model_size_suffix = 'c'
#             print("   ℹ️ Для YOLOv9 розміри 'n' та 's' замінено на 'c'.")
#         # ------------------------------------

#         training_mode = self._ask_training_mode()
#         config = {}
#         base_name = 'YOLOv8' if model_choice == '1' else 'YOLOv9'
#         weights_prefix = f'yolo{ "v8" if model_choice == "1" else "v9" }{model_size_suffix}'

#         if training_mode == 'finetune':
#             config['name'] = f"{base_name}{model_size_suffix.upper()} (Fine-tune)"
#             config['weights'] = f"{weights_prefix}.pt"
#             print(f"✅ Обрано донавчання. Модель почне з ваг '{config['weights']}'.")
#         else:
#             config['name'] = f"{base_name}{model_size_suffix.upper()} (Full)"
#             config['weights'] = f"{weights_prefix}.yaml"
#             print(f"✅ Обрано повне навчання з архітектури '{config['weights']}'.")
...
#         # Примусово вимикаємо аугментацію Mosaic, встановлюючи її ймовірність на 0 - кажуть це все ж таки погано
...
#         self.params['save_period'] = self.params['save_period']
