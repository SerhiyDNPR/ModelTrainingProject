...
def log_dataset_statistics_to_tensorboard(dataset, writer: SummaryWriter):
    ...
    print("\n💡 **ПОРАДА:** Тепер, коли ви запустите `tensorboard --logdir ./runs` і виберете кілька запусків, "
          "ви побачите накладені одна на одну лінії, що відображають форму розподілів для кожного експерименту.")